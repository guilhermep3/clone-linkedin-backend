...
export const followToggle = async (req, res) => {
    const usernameLogged = req.usernameLogged;
...
export const addSkills = async (req, res) => {
    ...
    const user = await findUserByUsername(req.usernameLogged);
    if (!user) {
        res.status(404).json({ error: 'Usuário não encontrado' });
        return;
    }
    const newSkills = await createSkills(safeData.data.name, safeData.data.level, user.id);
    res.json({ newSkills });
};
...
export const addExperienceSkill = async (req, res) => {
    ...
    const newExperienceSkill = await createExperienceSkill(safeData.data.experience_id, safeData.data.user_skill_id);
...
export const updateUser = async (req, res) => {
    ...
    if (req.body.username && req.body.username !== req.usernameLogged) {
        const newToken = createJWT(req.body.username, req.accountType);
        return res.json({ message: "Username atualizado", token: newToken });
    }
    const userUpdated = await updateUserByUsername(safeData.data, req.usernameLogged);
...
export const updateExperience = async (req, res) => {
    ...
    const experienceUpdated = await updateExperienceById(safeData.data, id);
...
export const updateUserSkills = async (req, res) => {
    ...
    const userSkillsUpdated = await updateUserSkillsById(safeData.data, id);
...
export const updateEducation = async (req, res) => {
    ...
    const educationUpdated = await updateEducationById(safeData.data, id);
...
export const updateCertificate = async (req, res) => {
    ...
    const certificateUpdated = await updateCertificateById(safeData.data, id);
...
export const deleteExperienceSkill = async (req, res) => {
    const id = parseInt(req.params.id);
    try {
